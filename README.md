...
- 🔄 полностью локальное разворачивание повторяющихся событий (RRULE/EXDATE) + сортировка списков встреч от ранних к поздним
- 📨 ежедневный дайджест "Все встречи на сегодня" в 09:00 по рабочей таймзоне (час задаётся через `DAILY_DIGEST_HOUR`)
- 📡 CalDAV REPORT теперь запрашивает расширенные экземпляры и надёжнее фиксирует отмены/переносы
- 🧪 улучшенная диагностика: в логах видно статусы REPORT, доступна кнопка RAW CalDAV
- 📘 документация обновлена для сценариев Docker/Portainer и быстрого запуска v1.1.0
...
- ✅ **Ежедневный дайджест** - автоматический обзор всех встреч в указанное время
...
- 📨 **Дайджест** - автоматически приходит всем авторизованным пользователям каждый день в указанное время
...
| `DAILY_DIGEST_HOUR` | Час отправки дайджеста (0-23) | ❌ | `9` |
...
4. **Проверка уведомлений** - Каждые 60 секунд (по умолчанию) строго на отметках `:00`
...
**Версия:** 1.1.0 · см. тег `v1.1.0` для фиксации релиза.
